...
I introduce a coheisiveness score (comprised of descriptive scores rather than confidence scores) to determine the overall sound of each album, this score is defined as follows:
...
We can see that only three songs in the entire corpus are spoken word songs according to Spotify. Eilart Pilram, B.J. Snowden, The Shaggs, and Wesley Willis all refrained from making spoken word tracks.
...
We can see that six songs in the entire corpus are instrumental songs songs according to Spotify. Eilart Pilram, The Shaggs, and Wesley Willis all refrained from making instrumental music.
...
compmus_long_distance(
  ...
  geom_tile() +
  coord_equal() +
  labs(title="Dynamic Time Warping: Silent Night",x = "Eilert Pilram", y = "Elvis Presley") +
  theme_minimal() +
...
mean_eilert <- eilert_playlist %>%
  summarise(artist_name = "Eilert Pilram",
            valence = mean(valence),
            energy  = mean(energy),
            danceability = mean(danceability),
            acousticness = mean(acousticness),
...
outsiders <-
  ...
  bind_rows(eilert %>% mutate(artist = "Eilert Pilram"))
...
word <- world %>%
  ...
  geom_raster() +
  scale_fill_viridis_c(guide = "none") +
  labs(title="Philophy of the World", x = "Time (s)", y = "Tempo (BPM)") +
  theme_classic()
...
do <- do %>%
  ...
  geom_raster() +
  scale_fill_viridis_c(guide = "none") +
  labs(title="Wat Should I Do?", x = "Time (s)", y = "Tempo (BPM)") +
  theme_classic()
...
***
The fact that each member of the Shaggs is playing a different song is clearly visible form the tempograms of 
...
enter <- enter %>%
  ...
  geom_tile() +
  labs(title="Chromagram: Sorry Entertainer, Daniel Johnson", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
...
shoes <- shoes %>%
  ...
  geom_tile() +
  labs(title="Chromagram: Worried Shoes, Daniel Johnson", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
...
columbo <- columbo %>%
  ...
  geom_tile() +
  labs(title="Chromagram: Who Killed John Columbo, Wesley Willis", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
...
easy <- easy %>%
  ...
  geom_tile() +
  labs(title="Chromagram: Living Is Easy, B.J. Snowden", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
```
...
halloween <- halloween %>%
  ...
  geom_tile() +
  labs(title="Chromagram: It's Halloween, The Shaggs", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
...
dad <- dad %>%
  ...
  geom_tile() +
  labs(title="Chromagram: Daddy, Daddy What Is Heaven Like, Tiny Tim", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
...
### All outdider musicians are **unique**, this means that the genre outsider music **cannot be generalized** trough their track features. 

```{r eval=FALSE, include=FALSE}
sun <- get_tidy_audio_analysis("1PmXm1881bonBI1AlG5uaH")
...
```{r eval=FALSE, include=FALSE}
tulips <- get_tidy_audio_analysis("1Vchex0xowRj9k59RLvRfo")
...
```
```{r eval=FALSE, include=FALSE}
near <- get_tidy_audio_analysis("2WhRJwrsy57H5Mm1P3lykJ")
...
```{r eval=FALSE, include=FALSE}
canada <- get_tidy_audio_analysis("25dIBpehsuaeHlALc09HBJ")
...
```
```{r eval=FALSE, include=FALSE}
jesus <- get_tidy_audio_analysis("2Od8Vrqg0SAqVTLMHCMmWU")

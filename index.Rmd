...
### Outsiders **cannot be generalized** trough mood. (pt. 2)
...
# Eilart
sweden <-
  get_tidy_audio_analysis("3QqtLDOABCqzKVMd70aGJL") %>%
  select(segments) %>%
  unnest(segments) %>%
...
# Elvis
us <-
  get_tidy_audio_analysis("452KBpASS5lZLeJWX9Ixub") %>%
  select(segments) %>%
  unnest(segments) %>%
  select(start, duration, pitches)

compmus_long_distance(
  sweden %>% mutate(pitches = map(pitches, compmus_normalise, "manhattan")),
  us %>% mutate(pitches = map(pitches, compmus_normalise, "manhattan")),
  feature = pitches,
  method = "angular"
) %>%
  ggplot(
  ...
  geom_tile() +
  coord_equal() +
  labs(title="Dynamic Time Warping: Jailhouse Rock",x = "Eilart Pilram", y = "Elvis Presley") +
  theme_minimal() +
  scale_fill_viridis_c(guide = NULL)

```

### The **Swedish Elvis** is pretty similar to his American counterpart.
```{r}
eilart_song <- subset(eilert_album, track_name=="Jailhouse Rock - 1998")
elvis_song <- get_track_audio_features("1w94wmfyOvNNhtZw7bv0gP", authorization = get_spotify_access_token())

jailhouse <- list(eilart_song, elvis_song) %>%
  reduce(full_join) %>%
  summarise(track_name,
            artist_name,
            valence=valence,
            energy=energy,
            danceability,
...
#eilert_data <- eilart_song[c("danceability", "valence", "speechiness", "acousticness", "instrumentalness", "liveness")]
#radarchart(eilert_data)

...
song_mood_plot_disc <- song_mood_plot + 
                 annotate("text", x=.25, y=.25, label="Sad", alpha=.6) +
                 annotate("text", x=.25, y=.75, label="Relaxed", alpha=.6) +
                 annotate("text", x=.75, y=.25, label="Angry", alpha=.6) +
                 annotate("text", x=.75, y=.75, label="Happy", alpha=.6) 
...
To examine if outsiders can be catgorized by mood, I plotted the mean valence and energy of each album to determine the albums mood (relaxed, happy, angry or sad). Spotify defines these paramaters as follows:
...
We can see that only three songs in the entire corpus are spoken word songs according to Spotify. 
...
```
***
Dynamic Time Warping is a technique in MIR used to compare pitches of two versions of a song into one single visualisation. If the two versions are similar a diagonal line should become visible.  
...
```{r}
beatles <-
  get_tidy_audio_analysis("68EnKIN8yb1Rh0EmsTZgRr") %>%
  select(segments) %>%
  unnest(segments) %>%
  select(start, duration, pitches)
...
beatles %>%
  ...
  geom_tile() +
  labs(title="Chromagram: The Beatles, Daniel Johnson", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
...
```{r}
kong <-
  get_tidy_audio_analysis("2QQovIO9Vblm1Ka9IWcTDV") %>%
  select(segments) %>%
  unnest(segments) %>%
  select(start, duration, pitches)
...
kong %>%
  ...
  geom_tile() +
  labs(title="Chromagram: King Kong, Daniel Johnson", x = "Time (s)", y = NULL, fill = "Magnitude") +
  theme_minimal() +
  scale_fill_viridis_c()
...
### **Daniel Johnston** is all over the place. (pt. 3)
...
### **Daniel Johnston** is all over the place. (pt. 4)
...
### **Daniel Johnston** is all over the place. (pt. 5)
